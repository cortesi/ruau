...
///
/// The public slot/generation/heap accessors are diagnostics only. The engine
...
impl RegistryRef {
    ...
    #[doc(hidden)] // Engine-internal handle minting; not an embedder API.
    #[must_use]
    pub fn from_parts(slot: u32, generation: u32, heap: HeapId) -> Self {
        Self {
            slot,
            generation,
            heap,
...
/// discourages a host from moving a heap argument into a `'static`
/// [`HostCall::Pending`] future. The brand is ergonomic, not a hard guarantee —
/// [`Gc::raw`] can still extract the handle. What actually keeps a stale or forged
/// handle out of the VM is the return contract: both a synchronous and an async
...
pub trait HostContext {
    ...
    /// range. The `'_` lifetime discourages moving the view into a `'static`
    /// [`HostCall::Pending`] future, but it is not a hard barrier ([`Gc::raw`]
    /// extracts the handle); a host that awaits must pin what it needs
