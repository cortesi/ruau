...
impl<'source> Parser<'source> {
    ...
    pub(crate) fn push_error_dedup(&mut self, error: Error) {
        if self
            .errors
            .iter()
            .any(|existing| existing.location == error.location)
        {
            return;
        }
        self.errors.push(error);
...
pub fn parse_number(text: &str) -> JsonNumber {
        ...
        .or_else(|| cleaned.strip_prefix("0X"))
    {
        ...
        cleaned.parse::<u64>().ok().map(serde_json::Number::from)
    };
...
pub fn number_literal_is_malformed(text: &str) -> bool {
    ...
    if cleaned.contains(['.', 'e', 'E']) {
        return cleaned.parse::<f64>().is_err();
    }

    cleaned.parse::<u64>().is_err()
}
...
/// Encodes upstream byte strings into the JSON-compatible string convention.
...
pub fn class_has_json_visible_members(class: &Stat) -> bool {
    let Stat::Class { members, .. } = class else {
        return false;
    };
