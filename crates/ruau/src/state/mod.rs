...
impl<T: AsChunk> ChunkInput<T> {
        ...
        T: 'a,
    {
        ...
        Chunk {
            ...
            mode: ChunkMode::Text,
            source: chunk.source(),
            // SAFETY: short-lived shared borrow of extra; no overlap with extra_mut().
            compiler: unsafe { lua.raw().extra() }.compiler.clone(),
        }
    }
}
...
impl Luau {
    ...
    pub fn new_with(libs: StdLib, options: LuauOptions) -> Result<Self> {
        ...
        Self::new_with_unchecked(libs, options)
    }
    ...
    /// # Security
    ///
    /// The caller should ensure loaded libraries are appropriate for the code that will run in
    /// this VM. In particular, [`StdLib::DEBUG`] exposes APIs that can break sandbox isolation.
    pub fn new_with_unchecked(libs: StdLib, options: LuauOptions) -> Result<Self> {
    ...
    pub fn load_std_libs(&self, libs: StdLib) -> Result<()> {
        let libs = libs
            .require_safe()
            .map_err(|error| Error::runtime(error.to_string()))?;
        self.load_std_libs_unchecked(libs)
    }
    ...
    /// The caller should ensure loaded libraries are appropriate for the code that will run in
    /// this VM. In particular, [`StdLib::DEBUG`] exposes APIs that can break sandbox isolation.
    pub fn load_std_libs_unchecked(&self, libs: StdLib) -> Result<()> {
    ...
    pub(crate) fn sandbox(&self, enabled: bool) -> Result<()> {
        ...
        unsafe {
            if lua.extra_mut().sandboxed != enabled {
                ...
                lua.extra_mut().sandboxed = enabled;
            }
            Ok(())
        }
    }
    ...
    /// Wraps a crate-owned C function trampoline, creating a callable Luau function handle to it.
    pub(crate) fn create_c_function(&self, func: ffi::lua_CFunction) -> Result<Function> {
        let lua = self.raw();
        lua.scoped_op(3, |state| {
    ...
    pub fn register_userdata_type<T: 'static>(
        ...
        unsafe {
            ...
            (lua.extra_mut().pending_userdata_reg).insert(type_id, registry.into_raw());
        }
        Ok(())
    }
    ...
    pub fn app_data_ref<T: 'static>(&self) -> Option<AppDataRef<'_, T>> {
        ...
        // SAFETY: Going through the raw pointer here keeps `extra`'s lifetime independent of
        // `guard`, so `guard` can be moved into `borrow(Some(guard))` while still using `extra`.
