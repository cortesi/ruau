...
impl Table {
    ...
    /// If they are not primitively equals, then ruau will try to invoke the `__eq` metamethod.
    /// ruau will check `self` first for the metamethod, then `other` if not found.
    ///
    /// # Examples
    ///
    ...
    ///
    /// If `key` is an integer, ruau shifts down the elements from `table[key+1]`,
