...
- `unsafe_op_in_unsafe_fn = "warn"`. The high-level crate still carries a
  crate-level allow pending the separate per-operation sweep described above.
- `clippy::undocumented_unsafe_blocks = "deny"`. Every audited
  `unsafe { ... }` block must have a `SAFETY:` comment.
- `clippy::missing_safety_doc = "warn"`. The high-level crate still carries a
  crate-level allow pending a per-function `# Safety` rustdoc sweep.
...
- `crates/ruau/src/lib.rs` keeps `#![allow(unsafe_op_in_unsafe_fn)]` and
  `#![allow(clippy::missing_safety_doc)]` pending the separate mechanical
  sweeps. It does not allow undocumented unsafe blocks.
