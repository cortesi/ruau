...
1. Stage One: Rename Legacy Luau Wording

The public API now uses Luau naming as a clean break. Legacy Lua-facing names are not retained as
compatibility aliases.

1. [x] Audit module-level docs for generic "Lua runtime" language that should say Luau.
2. [x] Decide whether crate naming and exported type names remain compatibility aliases or get a
   larger rename in a separate breaking-change pass.
3. [x] Rename internal `mlua_*` macros, `__mlua_*` registry keys, and old diagnostic text once
   public crate renaming is settled.
4. [x] Retire or rewrite the upstream `docs/release_notes` files so they no longer describe the
...
4. [x] Rework async examples around `tokio::task::LocalSet`, single-owner request handling, or
   per-connection `Luau` states instead of only making them compile with async calls.
