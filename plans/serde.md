...
- `Luau::create_serializable_userdata`,
  `Luau::create_serializable_opaque_userdata`, and `AnyUserData::wrap_ser`
  were removed instead of kept as deprecated shims.
- The old `UserDataVariant::Serializable` storage branch and `erased-serde`
  dependency were removed.
- Remaining userdata unsafe sites are still the expected Luau boundary work:
  stack/userdata pointer access, C callbacks and destructors, scoped userdata
  raw pointers, and lock guard lifetime bridging. The erased-serde pointer cast
  from `Box<dyn Serialize>` back to `T` is gone.

1. Stage One: Documentation And Surface Audit

Confirm the current public contract before changing behavior.

1. [x] Update `README.md` and examples to stop describing `serde` as an optional
   feature.
2. [x] Audit public serde-facing exports in `crates/ruau/src/lib.rs` and the
   ...
   crate-root export.
3. [x] Audit serializable-userdata entry points:
   `Luau::create_serializable_userdata`,
...
1. [x] Add rustdoc guidance to `serde` docs explaining when to use
   `to_value`/`from_value` versus userdata.
2. [x] Add rustdoc guidance to `UserData` explaining that serde does not replace
   userdata when object identity, methods, mutation, scoped borrows, destructors,
   or proxies are required.
3. [x] Review `examples/serde.rs` and `examples/userdata.rs` so they demonstrate
...
4. [x] Add or adjust tests that show serde round-tripping plain data does not
   require userdata.
...
1. [x] Extend the existing `UserDataRegistry<T>` with an explicit serde opt-in
   for `T: serde::Serialize`; do not introduce a parallel registry type.
2. [x] Store userdata created through the new registration-based serde path as
   concrete `T` regardless of whether serde support is enabled.
3. [x] Move serde dispatch for userdata from storage variants to type metadata,
   registry callbacks, or metatable-owned callbacks installed by
   `UserDataRegistry<T>`.
4. [x] Replace the current `Serialize for UserDataStorage<()>` implementation
...
5. [x] Remove `create_serializable_userdata`,
   `create_serializable_opaque_userdata`, and `AnyUserData::wrap_ser` before
   removing the old serializer branch, so no live public constructor can create
   `Serializable`-variant storage after that branch is gone.
6. [x] Replace `AnyUserData::is_serializable` and
...
7. [x] Preserve existing behavior for `Value::UserData(ud)` serialization:
...
8. [x] Preserve existing behavior for deserializing serde-enabled userdata from
   `Value::UserData(ud)`, including the current `serde/de.rs` paths that treat
   serializable userdata as a deserializable value.
9. [x] Decide at the end of this stage whether the constructor APIs will be
...
10. [x] Decide and document the migration path for opaque serde userdata whose
    ...
    workflow.
11. [x] Add tests covering borrow, borrow_mut, take, destroy, serialization, and
...
1. [x] Replace internal uses of `create_serializable_userdata`,
   `create_serializable_opaque_userdata`, and `AnyUserData::wrap_ser` with the
   new registration-based model where possible.
2. [x] Do not keep deprecated shims; Stage Three chose removal rather than a
   temporary compatibility layer.
3. [x] If Stage Three chose removal, remove the serializable constructors and
   update call sites, docs, and tests to use the canonical path chosen for
   opaque `T: Serialize` userdata.
4. [x] Remove tests that exist only to validate the old constructor split, or
...
1. [x] Review whether `ruau::value::SerializableValue` should remain public,
...
2. [x] Keep `SerializeOptions` and `DeserializeOptions` if they remain the
   clearest way to control nulls, array metatables, sorted keys, mixed tables,
   recursive tables, and unsupported types.
3. [x] Ensure option names no longer read as compatibility shims for an optional
   feature.
4. [x] Update serde tests to cover the final public option surface.
...
1. [x] Remove the `UserDataVariant::Serializable` branch and the raw pointer cast
   in `crates/ruau/src/userdata_impl/cell.rs` after Stage Four has moved or removed
...
2. [x] Remove `erased-serde` if no longer needed after userdata serialization
...
3. [x] Re-run the unsafe audit for userdata modules and document the remaining
   unsafe sites that are still required for Luau stack/userdata handling.
4. [x] Run the focused userdata and serde tests, then the project-standard full
   test command.
